...
namespace dsak
{
    namespace tcp
    {
        ...
        simsignal_t DSAK_Tcp::tcpConnectionAddedSignal = registerSignal("tcpConnectionAdded");
        simsignal_t DSAK_Tcp::tcpConnectionRemovedSignal = registerSignal("tcpConnectionRemoved");

        DSAK_Tcp::~DSAK_Tcp()
        {
        }

        void DSAK_Tcp::initialize(int stage)
        {
        ...
        void DSAK_Tcp::sendFromConn(cMessage *msg, const char *gatename, int gateindex)
        {
            Enter_Method_Silent();
            take(msg);
        ...
        void DSAK_Tcp::handleUpperPacket(Packet *packet)
        {
            handleUpperCommand(packet);
        }

        DSAK_TcpConnection *DSAK_Tcp::findConnForApp(int socketId)
        {
            auto i = tcpAppConnMap.find(socketId);
            return i == tcpAppConnMap.end() ? nullptr : i->second;
        }

        void DSAK_Tcp::handleLowerPacket(Packet *packet)
        {
            ...
            if (protocol == &Protocol::tcp)
            {
                ...
                // process segment
                DSAK_TcpConnection *conn = findConnForSegment(tcpHeader, srcAddr, destAddr);
                if (conn)
                {
        ...
        DSAK_TcpConnection *DSAK_Tcp::createConnection(int socketId)
        {
        ...
        void DSAK_Tcp::removeConnection(DSAK_TcpConnection *conn)
        {
        ...
        DSAK_TcpConnection *DSAK_Tcp::findConnForSegment(const Ptr<const TcpHeader>& tcpseg, L3Address srcAddr, L3Address destAddr)
        {
            SockPair key;
        ...
        void DSAK_Tcp::segmentArrivalWhileClosed(Packet *packet, const Ptr<const TcpHeader>& tcpseg, L3Address srcAddr, L3Address destAddr)
        {
            auto moduleType = cModuleType::get("ddosattacks.hackedmodules.transportlayer.tcp.DSAK_TcpConnection");
        ...
        ushort DSAK_Tcp::getEphemeralPort()
        {
        ...
        void DSAK_Tcp::addForkedConnection(DSAK_TcpConnection *conn, DSAK_TcpConnection *newConn, L3Address localAddr, L3Address remoteAddr, int localPort, int remotePort)
        {
        ...
        void DSAK_Tcp::addSockPair(DSAK_TcpConnection *conn, L3Address localAddr, L3Address remoteAddr, int localPort, int remotePort)
        {
        ...
        void DSAK_Tcp::updateSockPair(DSAK_TcpConnection *conn, L3Address localAddr, L3Address remoteAddr, int localPort, int remotePort)
        {
        ...
        TcpSendQueue *DSAK_Tcp::createSendQueue()
        {
            return new TcpSendQueue();
        }

        TcpReceiveQueue *DSAK_Tcp::createReceiveQueue()
        {
            return new TcpReceiveQueue();
        }

        void DSAK_Tcp::handleStartOperation(LifecycleOperation *operation)
        {
            //FIXME implementation
        }

        void DSAK_Tcp::handleStopOperation(LifecycleOperation *operation)
        {
        ...
        void DSAK_Tcp::handleCrashOperation(LifecycleOperation *operation)
        {
            reset();
        }
        ...
        bool DSAK_Tcp::checkCrc(Packet *packet)
        {
            auto tcpHeader = packet->peekAtFront<TcpHeader>();
        ...
        void DSAK_Tcp::refreshDisplay() const
        {
        ...
        std::ostream& operator<<(std::ostream& os, const DSAK_Tcp::SockPair& sp)
        {
        ...
        std::ostream& operator<<(std::ostream& os, const DSAK_TcpConnection& conn)
        {
            os << "socketId=" << conn.socketId << " ";
            os << "fsmState=" << DSAK_TcpConnection::stateName(conn.getFsmState()) << " ";
